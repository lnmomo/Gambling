...
def validate_profit_scorer_on_official_sp(
    ...
    for row in rows:
        ...
        profit = _profit("draw", float(mapped["odds"]), row.get("outcome")) if passes else None
        scored.append({
    ...
    return {
        "method": "prospective official-SP validation for market-anchored I2 profit scorer",
