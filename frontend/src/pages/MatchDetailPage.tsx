...
export default function MatchDetailPage() {
  ...
  const backtest = demoBacktestResult.records.find(item => item.matchId === matchId);
  if (backtest) return <div className="page"><div className="detail-hero"><div><Link to="/backtest">返回回测</Link><p>{backtest.league} · {backtest.officialMatchId}</p><h1>{backtest.homeTeam} <em>vs</em> {backtest.awayTeam}</h1><span>{new Date(backtest.kickoffTime).toLocaleString("zh-CN")}</span></div><RiskTag level={backtest.riskLevel} /></div><section className="panel"><div className="panel-heading"><div><h2>赛后诊断</h2><p>将赛前预测、真实赛果、收益、CLV 与概率误差放在同一记录中复核</p></div></div><div className="summary-strip backtest-inner"><span>推荐方向<b>{backtest.recommendation}</b></span><span>最终概率<b>{backtest.selectedProbability === undefined ? "-" : pct(backtest.selectedProbability)}</b></span><span>推荐时 SP<b>{backtest.selectedOfficialSp?.toFixed(2) ?? "-"}</b></span><span>EV<b>{backtest.ev === undefined ? "-" : pct(backtest.ev)}</b></span><span>建议 Stake<b>{backtest.stake.toFixed(2)}</b></span><span>真实赛果<b>{backtest.actualResult ?? "未完赛"}</b></span><span>是否命中<b>{backtest.hit === null ? "-" : backtest.hit ? "是" : "否"}</b></span><span>Profit<b>{backtest.profit.toFixed(2)}</b></span><span>收盘 SP<b>{backtest.selectedClosingSp?.toFixed(2) ?? "-"}</b></span><span>CLV<b>{backtest.clv === undefined ? "-" : pct(backtest.clv)}</b></span><span>Brier<b>{backtest.brierScore.toFixed(4)}</b></span><span>Log Loss<b>{backtest.logLoss.toFixed(4)}</b></span></div><div className="backtest-diagnostics"><p>{backtest.hit === false && backtest.clvPositive ? "本场未命中，但推荐价格优于收盘价格，短期结果可能受随机性影响。" : backtest.hit === false && backtest.clvPositive === false ? "本场未命中且 CLV 为负，模型可能未能提前识别真实市场方向。" : (backtest.ev ?? 0) >= .08 && backtest.hit === false ? "高 EV 推荐失败，需要在 EV 分桶分析中观察是否存在系统性过度自信。" : "本场结果应结合长期校准、ROI 与 CLV 样本共同评估。"}</p></div><div className="two-column backtest-inner"><div><h3>Critic 原因</h3>{backtest.noBetReason?.map(reason => <p key={reason}>{reason}</p>)}</div><div><h3>Warnings</h3>{backtest.warnings?.map(warning => <p key={warning}>{warning}</p>)}</div></div></section></div>;
