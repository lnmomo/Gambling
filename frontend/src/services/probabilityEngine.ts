...
export {calculateDynamicEvThreshold, runCriticCheck} from "../algorithm/criticRules";
...
import {allowedOddsAgeMinutes} from "../algorithm/probabilityEngine";

export function calculatePoisson1x2(lambdaHome: number, lambdaAway: number) {
  return calculateDixonColes1X2(lambdaHome, lambdaAway).probability;
}
...
export function calculateModelDisagreement(...models: ThreeWayProbability[]): number {
  return details(models).maxDisagreement;
}

export function runCriticChecks(match: Pick<OfficialMatch, "officialMatchId" | "status" | "officialSp" | "updatedAt" | "kickoffTime">, draft: {finalProbability: ThreeWayProbability; ev: ThreeWayEv; modelDisagreement: number | ModelDisagreement}, context: MatchContext = {}) {
  const disagreement = typeof draft.modelDisagreement === "number" ? {
    homeDisagreement: draft.modelDisagreement,
    drawDisagreement: draft.modelDisagreement,
    awayDisagreement: draft.modelDisagreement,
    maxDisagreement: draft.modelDisagreement,
    level: draft.modelDisagreement > .12 ? "HIGH" as const : draft.modelDisagreement > .07 ? "MEDIUM" as const : "LOW" as const,
  } : draft.modelDisagreement;
  const effectiveContext = {...context};
  const allowedAge = allowedOddsAgeMinutes(match.kickoffTime);
  const age = Date.now() - new Date(match.updatedAt).getTime();
  if (!Number.isFinite(age) || age > allowedAge * 60_000) effectiveContext.dataFreshness = "STALE";
  const report = runCriticCheck(match, {...draft, modelDisagreement: disagreement}, effectiveContext, .05);
  const reasons = [...report.reasons];
  if (effectiveContext.dataFreshness === "STALE") reasons.push(`官方赔率更新时间超过动态阈值${allowedAge}分钟`);
